...
export class FixedpluginComponent implements OnInit {
    ...
    async performActionButtonClicked() {
        ...
        await firebase.firestore().collection("incoming-calls").doc(this.data.id).update({"assigned_to": email})

        if(this.data.hasAccount) {
            location.href = `/new-ticket?customer=${this.data.customer_email}`
            return
        }
        location.href = `/new-ticket?create=new&number=234${this.data.params.phone_number}`
    }
